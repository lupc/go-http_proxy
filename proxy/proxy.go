...
type Option struct {
	...
	ApiHost string `yaml:"apiHost"` // API主机（域名或者IP+Port）
}

func init() {
	logger = myzap.NewDefaultLogger()
}

func LoadConfig(cfgPath string) {
	// var cfgPath = "config.yaml"
	dataBytes, err := os.ReadFile(cfgPath)
	if err == nil {

		VerOption = Option{}
		err = yaml.Unmarshal(dataBytes, &VerOption)
		if err != nil {
			logger.Sugar().Errorf("解析%v失败,%v", cfgPath, err)
		}
	}

	cfgContent, err := yaml.Marshal(VerOption)
	if err == nil {
		os.WriteFile(cfgPath, cfgContent, 0777)
	}
...
func Proxy(option Option) {
	...
	logger.Info(fmt.Sprintf("服务运行中... 端口[%v]", option.Port))
...
func doProxy(w http.ResponseWriter, r *http.Request, option Option) {
	...
	if err != nil {
		logger.Sugar().Errorf("读取请求体发生错误!%v", err)
		// 响应状态码
	...
	if err != nil {
		logger.Sugar().Errorf("转发请求发生错误!%v", err)
		// 响应状态码
		w.WriteHeader(http.StatusServiceUnavailable)
		return
	}
	...
	if err != nil {
		...
		logger.Sugar().Errorf("读取响应体发生错误!%v", err)
		// 响应状态码
		w.WriteHeader(http.StatusServiceUnavailable)
		return
	}
	...
	if isGzipped {
		...
		if err != nil {
			...
			logger.Sugar().Errorf("创建gzip读取器发生错误!%v", err)
			// 响应状态码
			w.WriteHeader(http.StatusServiceUnavailable)
			return
		}
		...
		if err != nil {
			...
			logger.Sugar().Errorf("读取gzip对象内容发生错误!%v", err)
			// 响应状态码
			w.WriteHeader(http.StatusServiceUnavailable)
			return
		}
